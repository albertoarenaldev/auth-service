...
# Las rutas /oauth2/authorization/* siempre existen (el centinela "NONE"
# en application.properties evita el boot-loop). Sin estas env vars, las
# rutas redirigen a GitHub/Google con client_id=NONE (el provider muestra
# su propio error). Con credenciales reales, OAuth2 funciona normalmente.
#
# NOTA: los nombres de variable usan CLIENTID (sin guion bajo) porque
# Spring Boot relaxed binding traduce _ → . (CLIENT_ID → client.id, que
# no es la propiedad correcta). Usar CLIENTID para que mapee a client-id.
#
